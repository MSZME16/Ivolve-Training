...
![alt text](<screenshots/Jenkins lab.svg>)
...
4. OpenShift: Deploys the application in the mohamedmasry namespace (optinal).
...
4. OpenShift cluster with access to the mohamedmasry namespace
...
pipeline {
    ...
    stages {
        ...
        stage('kubernetes') {
            steps {
                withKubeCredentials(kubectlCredentials: [[caCertificate: '', clusterName: '', contextName: '', credentialsId: '9', namespace: 'mohamedmasry', serverUrl: '']]) {
...
```
https://github.com/MSZME16/Shared-library
```
...
```bash
oc create sa jenkins -n mohamedmasry
```
...
```sh
kubectl create token jenkins -n mohamedmasry
```
